...
const galleries = {
  captains: [['assets/gallery/captains-01-new.jpg','Факультет бизнеса «Капитаны»'],['assets/gallery/captains-02.jpg','Практический модуль: ведение переговоров'],['assets/gallery/captains-03.jpg','Образовательные возможности и стажировки выпускников'],['assets/gallery/captains-04.jpg','Командная работа и мероприятия сообщества']],
...
function contentPhoto([src,caption],index,wide=false){return `<figure class="content-photo ${src.endsWith('.png')?'is-logo':''} ${src.includes('captains-01-new')?'is-banner':''} ${wide?'wide':''}">
...
<section class="other-projects"><div class="project-section-head"><span>ЕЩЁ ПРОЕКТЫ</span><h2>Образование, лидерство и технологии</h2></div><div class="project-card-grid">${others.map(p=>`<article class="project-card project-${p.slug}"><div class="project-logo small" aria-label="Логотип проекта ${p.name}">${p.mark}</div><span>${p.tag}</span><h3>${p.name}</h3><p>${p.lead}</p><div class="mini-achievement"><b>Результат:</b> ${p.achievement}</div><div class="project-socials">${projectLinks(p.links)}</div></article>`).join('')}</div></section>
...
<button class="captains-back" type="button" data-module="demin">← Вернуться к Александру Дёмину</button>`,
  presentation: () => `<div class="presentation-deck">
<div class="deck-toolbar"><div><span>НОВЫЕ ЛЮДИ · ПРЕЗЕНТАЦИЯ</span><b>Презентация партии</b></div><button id="fullscreen-slide" type="button" aria-label="Открыть презентацию на весь экран">⛶ На весь экран</button></div>
...
function showCatalog(){catalog.classList.add('active');lesson.classList.remove('active','presentation-mode');document.onkeydown=null;history.replaceState(null,'','#catalog');document.querySelectorAll('.nav button').forEach(x=>x.classList.remove('active'));window.scrollTo(0,0)}
function openModule(id){const index=modules.findIndex(m=>m.id===id),isHidden=id==='captains';if(index<0&&!isHidden)return;catalog.classList.remove('active');lesson.classList.add('active');lesson.classList.toggle('presentation-mode',id==='presentation');content.innerHTML=lessons[id]();if(id==='links'){const personalContact=content.querySelector('a[href="https://t.me/ivinovino"]');if(personalContact){personalContact.href='https://t.me/newpeopleekb';personalContact.textContent='Telegram-канал'}}if(id==='demin'){embedDeminRoadmap();embedCaptainsEntry()}embedPhotos(id);document.querySelector('#lesson-number').textContent=isHidden?'Спецраздел':`Модуль ${index+1} из ${modules.length}`;document.querySelector('#progress-bar').style.width=isHidden?'50%':`${(index+1)/modules.length*100}%`;document.querySelectorAll('.nav button').forEach(x=>x.classList.toggle('active',x.dataset.module===id));history.replaceState(null,'',`#${id}`);wireLesson(id);document.querySelector('.nav').classList.remove('open');document.querySelector('.menu-button').setAttribute('aria-expanded','false');window.scrollTo(0,0)}
function wireLesson(id){document.onkeydown=null;if(id==='presentation'){let current=0,hideTimer;const image=document.querySelector('#slide-image'),num=document.querySelector('#slide-current'),thumbs=[...document.querySelectorAll('.slide-thumb')],deck=document.querySelector('.presentation-deck'),fullButton=document.querySelector('#fullscreen-slide');const set=n=>{current=(n+slides.length)%slides.length;image.src=slides[current];image.alt=`Слайд ${current+1}`;num.textContent=current+1;thumbs.forEach((t,i)=>t.classList.toggle('active',i===current));thumbs[current]?.scrollIntoView({block:'nearest',inline:'nearest'})};const scheduleThumbsHide=()=>{clearTimeout(hideTimer);deck.classList.remove('deck-thumbs-hidden');hideTimer=setTimeout(()=>deck.classList.add('deck-thumbs-hidden'),5000)};const closeExpanded=()=>{clearTimeout(hideTimer);deck.classList.remove('deck-expanded','deck-thumbs-hidden');document.body.classList.remove('deck-expanded-open');fullButton.textContent='⛶ На весь экран'};document.querySelector('#prev-slide').onclick=()=>set(current-1);document.querySelector('#next-slide').onclick=()=>set(current+1);fullButton.onclick=async()=>{if(deck.classList.contains('deck-expanded')){closeExpanded();return}if(document.fullscreenElement){await document.exitFullscreen?.();return}try{if(!deck.requestFullscreen)throw new Error('Fullscreen unavailable');await deck.requestFullscreen();scheduleThumbsHide()}catch(e){deck.classList.add('deck-expanded');document.body.classList.add('deck-expanded-open');fullButton.textContent='× Закрыть показ';scheduleThumbsHide()}};document.addEventListener('fullscreenchange',()=>{const active=!!document.fullscreenElement;fullButton.textContent=active?'× Закрыть показ':'⛶ На весь экран';if(active)scheduleThumbsHide();else{clearTimeout(hideTimer);deck.classList.remove('deck-thumbs-hidden')}},{once:false});thumbs.forEach(t=>t.onclick=()=>set(Number(t.dataset.slide)));document.onkeydown=e=>{if(e.key==='Escape'&&deck.classList.contains('deck-expanded'))closeExpanded();if(e.key==='ArrowLeft')set(current-1);if(e.key==='ArrowRight'||e.key===' ')set(current+1)}}document.querySelectorAll('.quiz-button').forEach(btn=>btn.onclick=()=>{const box=btn.closest('.check'),picked=box.querySelector('input:checked'),out=box.querySelector('.check-result');if(!picked){out.textContent='Выберите ответ.';return}out.textContent=picked.value===btn.dataset.answer?'Верно.':'Пока нет — перечитайте блок выше.';out.style.color=picked.value===btn.dataset.answer?'#08756e':'#b3293b'})}
